...
function Restore-MailSiteLegacyServiceConfiguration {
    ...
    $description = Get-UninstallerStateMapEntry -State $State -MapName 'PreviousDescription' -EntryName $ServiceName
    if ($description.Found) {
        & sc.exe description $ServiceName ([string]$description.Value) | Out-Null
        if ($LASTEXITCODE -ne 0) { throw "Cannot restore service description for $ServiceName." }
    }
    $startType = Get-UninstallerStateMapEntry -State $State -MapName 'UpgradeStartTypes' -EntryName $ServiceName
    if ($startType.Found) {
        $mode = switch ([int]$startType.Value) { 2 {'Automatic'} 3 {'Manual'} 4 {'Disabled'} default { throw 'Invalid saved service startup mode.' } }
