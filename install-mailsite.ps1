...
# Validated with the v11.0.227 release toolchain. Advance this floor when a
# release uses a newer MSVC runtime; never downgrade a newer installed runtime.
$VisualCppMinimumVersion = [version]"14.51.36247.0"
$script:LogPath = $null
...
function Get-FreshMailSitePendingListeners {
    # These are the fresh configuration's public and loopback protocol ports.
    # Do not apply this probe to upgrades with customized/disabled listeners.
    foreach ($port in @(25, 110, 143, 443, 18081, 18082, 18083)) {
        $client = [Net.Sockets.TcpClient]::new()
        try {
            if (-not $client.ConnectAsync('127.0.0.1', $port).Wait(250) -or -not $client.Connected) {
                $port
            }
        } catch {
            $port
...
function Wait-FreshMailSiteReadiness {
    param([int]$TimeoutSeconds = 120)

    Write-InstallerMessage "Waiting for MailSite's fresh-install listeners to become ready..."
    $timer = [Diagnostics.Stopwatch]::StartNew()
    do {
        $stopped = @($Services | Where-Object { (Get-Service -Name $_.Name -ErrorAction Stop).Status -ne 'Running' })
        if ($stopped.Count -gt 0) {
            throw "MailSite services stopped before becoming ready: $($stopped.Name -join ', ')."
        }
        $pending = @(Get-FreshMailSitePendingListeners)
        if ($pending.Count -eq 0) { return }
        if ($timer.Elapsed.TotalSeconds -ge $TimeoutSeconds) { break }
        Start-Sleep -Milliseconds 500
    } while ($true)
    throw "MailSite did not become ready within $TimeoutSeconds seconds. Pending listener ports: $($pending -join ', '). Check the service logs."
}
...
function Get-VisualCppRuntimeVersion {
    ...
    # Use explicit views so detection works from either PowerShell architecture.
    # Microsoft documents this key for the unified v14 redistributable:
    # https://learn.microsoft.com/en-us/cpp/windows/redistributing-visual-cpp-files
    $newest = $null
    foreach ($view in @([Microsoft.Win32.RegistryView]::Registry32, [Microsoft.Win32.RegistryView]::Registry64)) {
        $base = [Microsoft.Win32.RegistryKey]::OpenBaseKey([Microsoft.Win32.RegistryHive]::LocalMachine, $view)
        try {
            $key = $base.OpenSubKey("SOFTWARE\Microsoft\VisualStudio\14.0\VC\Runtimes\$Architecture")
            if ($null -eq $key) { continue }
            try {
                $version = $null
                $text = ([string]$key.GetValue("Version", "")).TrimStart('v', 'V')
                if ([int]$key.GetValue("Installed", 0) -eq 1 -and [version]::TryParse($text, [ref]$version)) {
                    if ($null -eq $newest -or $version -gt $newest) { $newest = $version }
                }
            } finally { $key.Dispose() }
...
function Test-VisualCppRuntimeRequired {
    ...
    function Read-PeNumber([long]$Offset, [int]$Size = 4) {
        if ($Offset -lt 0 -or $Offset + $Size -gt $data.LongLength) { throw 'Truncated PE image.' }
        if ($Size -eq 2) { return [BitConverter]::ToUInt16($data, [int]$Offset) }
    ...
    for ($index = 0; $index -lt $count; $index++) {
        ...
        $sections += @{Address=[long](Read-PeNumber ($section + 12)); Size=[long](Read-PeNumber ($section + 16)); Offset=[long](Read-PeNumber ($section + 20))}
    }
    function Resolve-PeAddress([long]$Address, [long]$Size) {
        foreach ($section in $sections) {
            $relative = $Address - $section.Address
            if ($relative -ge 0 -and $relative + $Size -le $section.Size) {
                $offset = $section.Offset + $relative
                if ($offset + $Size -gt $data.LongLength) { throw 'Truncated PE section.' }
                return $offset
            }
        }
    ...
    foreach ($directory in @(@(1,20,12), @(13,32,4))) {
        ...
        $offset = Resolve-PeAddress $rva $size
        $terminated = $false
        for ($position = $offset; $position + $stride -le $offset + $size; $position += $stride) {
            $nonzero = $false
            for ($j = 0; $j -lt $stride; $j += 4) { if ((Read-PeNumber ($position + $j)) -ne 0) { $nonzero = $true } }
            if (-not $nonzero) { $terminated = $true; break }
            if ($number -eq 13 -and (Read-PeNumber $position) -ne 1) { throw 'Unsupported PE delay import format.' }
            $nameRva = Read-PeNumber ($position + $nameOffset)
            $name = ''
            for ($j = 0; $j -lt 260; $j++) {
                $value = $data[(Resolve-PeAddress ($nameRva + $j) 1)]
                if ($value -eq 0) { break }
                $name += [char]$value
            }
            if ($name.Length -eq 0 -or $j -eq 260) { throw 'Invalid PE import name.' }
            if ($name -match '^(vcruntime|msvcp|concrt)') { $required = $true }
        }
...
function Install-VisualCppRuntime {
    ...
    $installed = Get-VisualCppRuntimeVersion -Architecture $Architecture
    if ($null -ne $installed -and $installed -ge $VisualCppMinimumVersion) {
        Write-InstallerMessage "Microsoft Visual C++ $Architecture runtime $installed is already installed."
        return
    }

    $url = "https://aka.ms/vc14/vc_redist.$Architecture.exe"
    $installerPath = Join-Path ([IO.Path]::GetTempPath()) ("MailSite-VC-$Architecture-" + [Guid]::NewGuid().ToString("N") + ".exe")
    try {
        Write-InstallerMessage "Installing Microsoft Visual C++ $Architecture runtime (minimum $VisualCppMinimumVersion)..."
        Invoke-WebRequest -Uri $url -OutFile $installerPath -UseBasicParsing -ErrorAction Stop
        $signature = Get-AuthenticodeSignature -LiteralPath $installerPath
        if ($signature.Status -ne "Valid" -or $null -eq $signature.SignerCertificate -or
            $signature.SignerCertificate.Subject -notmatch '(?:^|,\s*)O=Microsoft Corporation(?:,|$)') {
            throw "The downloaded runtime does not have a valid Microsoft signature."
        }

        $process = Start-Process -FilePath $installerPath -ArgumentList "/install", "/quiet", "/norestart" -WindowStyle Hidden -Wait -PassThru
        if ($process.ExitCode -eq 3010 -or $process.ExitCode -eq 1641) {
            throw "Windows requires a reboot to finish the runtime installation. Restart Windows, then rerun the MailSite installer."
        }
        # 1638 means another version is installed; accept it only if detection
        # confirms a sufficient version (e.g. another installer won a race).
        if ($process.ExitCode -notin @(0, 1638)) {
            throw "Runtime installer exited with code $($process.ExitCode)."
        }
        $installed = Get-VisualCppRuntimeVersion -Architecture $Architecture
        if ($null -eq $installed -or $installed -lt $VisualCppMinimumVersion) {
            throw "Runtime installer completed, but version $VisualCppMinimumVersion or newer was not detected."
        }
        Write-InstallerMessage "Microsoft Visual C++ $Architecture runtime $installed installed successfully."
    } catch {
        throw "Microsoft Visual C++ $Architecture runtime is required. Automatic installation failed: $($_.Exception.Message) Download $url if manual installation is needed, then rerun MailSite setup."
    } finally {
        Remove-Item -LiteralPath $installerPath -Force -ErrorAction SilentlyContinue
    }
}

function Install-MailSiteRuntimePrerequisites {
    param([string]$PackageRoot)

    foreach ($architecture in (Get-PackageRuntimeArchitectures -PackageRoot $PackageRoot)) {
        Install-VisualCppRuntime -Architecture $architecture
    }
...
function Resolve-InstallRequest {
    ...
    throw "Unknown install target '$target'. Use no argument for latest, 'reinstall' to replace the detected build (or use latest when the binaries conflict), or an exact version such as 11.0.112."
}
...
function Get-InstalledMailSiteComparisonVersion {
    ...
    # A mixed installation has no meaningful source version. Callers may
    # still replace it with a complete package, but must not classify that
    # repair as an upgrade or downgrade from an arbitrary component build.
    return $InstalledState.ExactVersion
}
...
function Get-MailSiteInstallPrompt {
    ...
    if ($needsRepair -and $null -ne $InstalledState -and $InstalledState.HasV11Artifacts) {
        return "Repair MailSite $installedDisplayVersion with MailSite ${TargetVersion}?"
    }
...
function New-InteractiveRemoteInstallRequest {
    ...
    return @{
        ...
        ForceReinstall = ($null -ne $InstalledState -and $InstalledState.HasV11Artifacts)
        Interactive = $true
        SkipConfirm = $true
        Cancelled = $false
...
function Assert-ExistingMailSite11Contract {
    ...
    if ($InstalledState.IsMixed) {
        Write-InstallerMessage "Existing MailSite 11 components are from mixed builds: $(Get-MailSiteComponentVersionSummary -InstalledState $InstalledState). The installation is damaged, but a complete MailSite package can replace it." -Level "WARN"
        return
    }
    if ((Test-ExactMailSiteVersion -Version $detectedVersion) -and $detectedVersion -ne $stateTarget) {
        Write-InstallerMessage "Existing MailSite 11 binaries report version '$detectedVersion', while install.json records '$stateTarget'. Using the coherent binary version as the installed version." -Level "WARN"
    }
}
...
function Write-MailSite11UpgradeWarning {
    ...
    Write-InstallerMessage "Upgrading MailSite $InstalledVersion to $TargetVersion in place. MailSite 11 is still in development; back up MailSite data first because MSDBMA may apply forward-only database migrations during indexing. Databases that cannot be migrated remain unopened and their indexing errors are shown in the Console DB view." -Level "WARN"
}
...
function Write-MailSite11DowngradeWarning {
    ...
    Write-InstallerMessage "Downgrading MailSite $InstalledVersion to $TargetVersion in place. Existing MailSite 11 databases are not downgraded. MSDBMA will automatically apply supported forward migrations when it indexes them; a database newer than this target build will remain unopened and its indexing error will be shown in the Console DB view." -Level "WARN"
}
...
function Save-FreshConsoleLogin {
    param([string]$DomainName, [string]$Password)

    # The Console owns its per-user handoff; passwords never enter process arguments.
    $start = [Diagnostics.ProcessStartInfo]::new()
    $start.FileName = Join-Path $InstallDir 'console.exe'
    $start.Arguments = '--configure-local-server'
    ...
    try {
        ...
        $payload = @{ username = "Postmaster@$DomainName"; password = $Password } | ConvertTo-Json -Compress
        ...
        if (-not $process.WaitForExit(30000)) {
            $process.Kill()
            throw 'Console credential handoff timed out.'
        }
        if ($process.ExitCode -ne 0) { throw 'Console credential handoff failed.' }
        Write-InstallerMessage "Saved the local Postmaster connection for this Windows user's first Console launch."
    } finally {
        $process.Dispose()
    }
}
...
function Install-MailSiteFresh {
    ...
    try {
        ...
        Install-MailSiteRuntimePrerequisites -PackageRoot $packageRoot
        ...
        if ($postmaster.Generated) {
            ...
            Write-Host "  Record this password for access from other clients or Windows accounts. It will not be shown again by setup." -ForegroundColor Yellow
        ...
        try {
            Save-FreshConsoleLogin -DomainName $domainName -Password $postmaster.Password
        } catch {
            Write-InstallerMessage 'Could not save the initial Console login. Enter the Postmaster login in Console manually.' -Level 'WARN'
        }
...
function Install-MailSite {
    ...
    if (-not $installedState.HasV11Artifacts) {
        ...
        if ($installedState.MissingComponents.Count -gt 0) {
            Write-InstallerMessage "Existing MailSite 11 install is missing component(s): $($installedState.MissingComponents -join ', '). A complete-package repair will replace the installed service set."
        }
        if ($installedState.InvalidComponents.Count -gt 0) {
            Write-InstallerMessage "Existing MailSite 11 install has invalid component(s): $($installedState.InvalidComponents -join ', '). A complete-package repair will replace the installed service set." -Level "WARN"
        }
    ...
    if (-not $recoverUpgrade -and $installedState.HasV11Artifacts -and $installRequest.ForceReinstall -and
        ...
        if (Test-ExactMailSiteVersion -Version $installedVersion) {
            ...
            # No single source build exists. Reinstalling the latest complete
            # package is the only deterministic repair for mixed binaries.
            Write-InstallerMessage "The installed MailSite binaries do not report one version. Reinstall will replace the damaged installation with the latest available build." -Level "WARN"
        }
    }
    ...
    if ($installedState.HasV11Artifacts) {
        ...
        if (-not $installRequest.ForceReinstall -and -not $needsRepair) {
            $serviceAccountAudit = Get-MailSiteServiceAccountAudit
            Write-MailSiteServiceAccountMismatchWarning -Audit $serviceAccountAudit
            Set-MailSiteInstallDataDirectoryAcl -RootDirectory $InstallDir -Audit $serviceAccountAudit
            [void](Invoke-MailSiteServiceControlPermissionRepair -HttpmaPath (Join-Path $InstallDir "httpma.exe"))
    ...
    try {
        ...
        # Publish recovery ownership before changing services or package files.
        ...
        foreach ($name in $state.UpgradeStartTypes.Keys) {
            if (Test-MailSiteServiceInstalled -ServiceName $name) {
                $mode = switch ([int]$state.UpgradeStartTypes[$name]) { 2 {'Automatic'} 3 {'Manual'} 4 {'Disabled'} }
        ...
        if ($null -ne $state -and $state.ContainsKey('UpgradeStartTypes')) {
            foreach ($name in $state.UpgradeStartTypes.Keys) {
                if (Test-MailSiteServiceInstalled -ServiceName $name) {
                    Set-Service -Name $name -StartupType Disabled
                    Stop-MailSiteService -ServiceName $name | Out-Null
                }
            }
        }
